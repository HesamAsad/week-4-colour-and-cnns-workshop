...
| slide | |
|---|---|
| 4 · three numbers from a spectrum | drag the peak and width; each of X, Y, Z is drawn as the **shaded overlap area** between the light and one sensitivity curve, so the integral is something you can see shrink |
| 5 · two lights, one colour | a smooth spectrum and a three-spike mixture, solved to identical XYZ and shown as two halves of one swatch — there is no seam. Push it far enough and the mixture needs a negative weight, which is a gamut boundary |
...
| 7 · there and back again | one space at a time, with the true channel ranges, plus the round trip |
| 8 · pick a pixel | click or drag on the macaws; the pixel is reported in all four spaces at once |
| 9 · the a\*–b\* plane | L\* slider; colours outside the sRGB gamut fade out, so you get a real gamut slice, and the caption reports how much of the square sRGB can reach |
| 10 · colour swap | R↔G in RGB vs inverting a\* in Lab, the lecture's own example |
| 14 · diffuse reflectance | light direction, intensity, reflectance and light colour, with the cos θ curve tracking alongside |
| 16 · underconstrained | R and I_L are traded with their product held fixed — the world changes, the image does not |
| 18 · which parameter is changing | the three chessboard images with a magnified inset and **click-to-reveal** answers |
| 23 · why divide by 255 | contours of the loss and the descent path; the button rescales and the zig-zag disappears |
| 24 · flatten | shuffle the 784 pixels and see that the MLP would not care |
| 26 · activations | ReLU, and a softmax whose confidence you can scale |
| 27 · a conv layer | a real learned 5×5 kernel sweeping a real MNIST digit, building its activation map |
| 29 · max pooling | 8×8 → 4×4 with every number visible and the winners highlighted |
| 31 · learned filters | the eight real first-layer kernels |
| 32 · the whole network | **the trained CNN, running live** — activation maps at both stages and the softmax bars |
...
`js/imaging.js` re-implements the forward pass, so slide 32 is genuinely running the network,
...
| 10 · colour swap | R↔G moves brightness; inverting a\* does not. Look at the shadows. |
| 11 · OpenCV traps | Hue is 0–179. Round trips are lossy. `BGR2XYZ` skips the gamma but `BGR2Lab` does not. |
| 14 · Lambertian | Camera position does not appear in the equation. That is what "diffuse" buys you. |
| 16 · underconstrained | Let the silence sit while the image refuses to change. |
| 18 · the exercise | Do not reveal until they commit. Image 3 has **two** answers. |
| 19 · edges | The hinge of the workshop: we argue for edges from physics, then watch a network find them. |
| 21 · last week vs this week | The convolution is unchanged. Only the source of the 25 numbers changed. |
| 24 · flatten | A CNN's advantage is a *constraint*, not extra capacity. |
| 25 / 30 · counting | Make them compute 784×16+16 and 5×5×8×16+16 before revealing. The bias and the input-channel factor are what people forget. |
| 30 · CNN params | `summary()` after training reports 17,984 — that includes Adam's state. The answer is 5,994. |
| 31 · learned filters | Be honest: they are *partly* edge detectors. All eight sum above zero, where Sobel sums to zero. |
| 33 · comparison | Quote errors, not accuracy: 460 vs 200 per 10,000, not 95% vs 98%. |
| 36 · take-homes | Card 4 (underconstrained ⇒ you must add an assumption) and card 8 (test set hygiene). |
...
- Part 3 uses **lectures 06–08**'s vocabulary: *activation map* (not "feature map" — worksheet 5
  uses the other name, and slide 27 says so explicitly), *tolerance* rather than invariance for
  pooling, and the note that Keras's `Conv2D` really computes cross-correlation, which lecture 6
  also flags.
- Slide 28 gives the output-size formula in the CS231n form \(\lfloor (W-F+2P)/S \rfloor + 1\)
